...
rats "require alias with stdlib name calls user module"
  ...
  HELPERS
  test.write_file(test.tmpdir() + "/t1/helpers.rg", helpers)
  main = <<~MAIN
    require "helpers" as "str"
    puts(str.upper("hello"))
  MAIN
  test.write_file(test.tmpdir() + "/t1/main.rg", main)
...
rats "required file imports propagate"
  ...
  HELPERS
  test.write_file(test.tmpdir() + "/t3/helpers.rg", helpers)
  main = <<~MAIN
    require "helpers"
    puts(helpers.double_str(21))
  MAIN
  test.write_file(test.tmpdir() + "/t3/main.rg", main)
...
rats "require inside function body errors"
  ...
  HELPERS
  test.write_file(test.tmpdir() + "/t5/helpers.rg", helpers)
  main = <<~MAIN
    def bar()
      require "helpers"
    end
  MAIN
  test.write_file(test.tmpdir() + "/t5/main.rg", main)
...
rats "duplicate namespace function errors"
  ...
  A
  test.write_file(test.tmpdir() + "/t6/a.rg", a)
  b = <<~B
    def foo()
      return "b"
    end
  B
  test.write_file(test.tmpdir() + "/t6/b.rg", b)
  main = <<~MAIN
    require "a" as "ns"
    require "b" as "ns"
    puts(ns.foo())
  MAIN
  test.write_file(test.tmpdir() + "/t6/main.rg", main)
...
rats "shared stdlib import across main and required file"
  ...
  main = <<~MAIN
    ...
    puts(conv.to_s(x + 5))
  MAIN
  test.write_file(test.tmpdir() + "/t7/main.rg", main)
  result = test.run("rugo run " + test.tmpdir() + "/t7/main.rg")
  test.assert_eq(result["status"], 0)
