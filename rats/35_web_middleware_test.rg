...
rats "real_ip strips port from remote_addr"
  result = test.run("rugo run rats/fixtures/web_real_ip_basic.rg")
...
rats "real_ip works before logger"
  result = test.run("rugo run rats/fixtures/web_real_ip_logger.rg")
  test.assert_eq(result["status"], 0)
...
rats "rate_limiter blocks after burst"
  result = test.run("rugo run rats/fixtures/web_rate_limit.rg")
  test.assert_eq(result["status"], 0)
  lines = result["lines"]
...
rats "rate_limiter recovers after waiting"
  result = test.run("rugo run rats/fixtures/web_rate_limit_recover.rg")
  test.assert_eq(result["status"], 0)
  lines = result["lines"]
  test.assert_eq(lines[0], "pong")
  test.assert_eq(lines[1], "pong")
...
rats "rate_limiter default 10 rps without config"
  result = test.run("rugo run rats/fixtures/web_rate_limit_default.rg")
  test.assert_eq(result["status"], 0)
  lines = result["lines"]
  test.assert_eq(lines[0], "pong")
  test.assert_eq(lines[1], "pong")
...
rats "rate_limiter compiles to native binary"
  ...
  SCRIPT
  test.write_file(test.tmpdir() + "/test.rg", script)
  result = test.run("rugo build " + test.tmpdir() + "/test.rg -o " + test.tmpdir() + "/mwbin")
  test.assert_eq(result["status"], 0)
...
rats "real_ip compiles to native binary"
  ...
  SCRIPT
  test.write_file(test.tmpdir() + "/test.rg", script)
  result = test.run("rugo build " + test.tmpdir() + "/test.rg -o " + test.tmpdir() + "/ipbin")
  test.assert_eq(result["status"], 0)
