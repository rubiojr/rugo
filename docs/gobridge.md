...
| `gobridge/path.go` | path package mappings (auto-generated by bridgegen) |
| `gobridge/html.go` | html package mappings (auto-generated by bridgegen) |
| `cmd/dev/bridgegen.go` | `rugo dev bridgegen` — scaffolds new bridge packages via `go/types` |
...
const (
    ...
    GoByteSlice                   // → []byte(rugo_to_string(arg)), returns as string
    GoInt64                       // → int64(rugo_to_int(arg)), returns as int
    GoRune                        // → rugo_first_rune(rugo_to_string(arg)), returns as string
    GoFunc                        // → lambda adapter (see FuncTypes)
