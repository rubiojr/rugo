# RATS: Test Go bridge — error handling and edge cases
use "test"
...
rats "Go bridge in required file"
  ...
  SCRIPT
  test.write_file(test.tmpdir() + "/helper.rg", helper)

  main = <<~SCRIPT
    require "helper"
    puts(helper.shout("hello"))
  SCRIPT
  test.write_file(test.tmpdir() + "/main.rg", main)

  result = test.run("rugo run " + test.tmpdir() + "/main.rg")
  test.assert_eq(result["status"], 0)
  test.assert_contains(result["output"], "HELLO")
end
